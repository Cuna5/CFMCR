...
class ConsistencyResidualDiffusionEngine(ResidualDiffusionEngine):
    """
    Training engine for Consistency Distillation of the Mean-Reverting Diffusion
    Model (EMRDM).

    Key idea
    --------
    A consistency model f_θ satisfies the self-consistency property:
    ...
    def _make_teacher_fn(self):
        ...
        Signature expected by ConsistencyResidualDiffusionLoss:
            teacher_fn(x_tn, sigma_n, sigma_n1, st_n, st_n1,
                       sigma2st, mu, cond, **extra) -> (x_tn1, f_target)

        Both returned tensors are detached from the student graph.
        """
        ...
        def teacher_fn(x_tn, sigma_n, sigma_n1, st_n, st_n1, _sigma2st, mu, cond, **extra):
            ...
            st_deriv       = _sigma2st.get_derivative_st()(sigma_n)
            st_deriv_bc    = append_dims(st_deriv, x_tn.ndim)
            sigma_n_bc     = append_dims(sigma_n,  x_tn.ndim)

            d = ((-st_deriv_bc) / (st_n_bc ** 2)) * mu - (
                denoised_n + (1.0 - st_n_bc) / st_n_bc * mu - x_tn
            ) / sigma_n_bc
