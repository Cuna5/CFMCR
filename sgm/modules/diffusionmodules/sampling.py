...
class ConsistencyResidualSampler(BaseResidualDiffusionSampler):
    """
    Single-step sampler for a trained consistency model.

    A consistency model f_θ satisfies:
        f_θ(x_{σ_n}, σ_n, μ) ≈ x_clean  for all σ_n

    Inference requires exactly **one** network call:
        1. Initialise  x_init = ((1-s_max)/s_max)·μ + σ_max·noise
        2. Return      f_θ(x_init, σ_max, μ)

    Compatible with the existing ResidualDiffusionEngine / log_images API.
    """
    ...
    def __call__(
        ...
        # ── sigma schedule (descending, ends with 0) ───────────────────────
        sigmas = self.discretization(
        ...
        # ── initialise noisy state at σ_max ───────────────────────────────
        st0 = self.sigma2st(sigma_max)
        x_init = ((1.0 - st0) / st0) * mu + sigma_max * x

        # ── single denoiser call ───────────────────────────────────────────
        st = self.sigma2st(sigma)
        denoised = self.denoise(x_init, denoiser, sigma, cond, st, uc)

        others = {}
        if return_intermediate:
            others["intermediates"] = [tools_scale(x_init.clone().detach())]
        if return_denoised:
            others["denoiseds"] = [tools_scale(denoised.clone().detach())]

        return denoised, others
